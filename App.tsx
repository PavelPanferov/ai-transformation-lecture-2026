  ...
  return (
    <div className="relative w-screen h-screen bg-slate-50 text-slate-900 overflow-hidden selection:bg-primary selection:text-white">
      ...
      <main className={`relative z-10 w-full h-full transition-opacity duration-300 ${isTransitioning ? 'opacity-0 scale-95 blur-sm' : 'opacity-100 scale-100 blur-0'}`}>
        <div className="absolute inset-2 md:inset-6 border border-white bg-white/50 backdrop-blur-md shadow-2xl overflow-hidden rounded-2xl ring-1 ring-black/5">
      ...
      {/* Navigation */}
      <div className="fixed bottom-0 left-0 w-full z-20 p-4 md:p-6 flex justify-between items-end pointer-events-none">
        <div className="flex gap-2 pointer-events-auto bg-white/80 p-2.5 rounded-full border border-slate-200 shadow-sm backdrop-blur-sm">
          {ALL_SLIDES.map((_, idx) => (
            <button key={idx} onClick={() => goTo(idx)}
              className={`h-2.5 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-10 bg-primary shadow-sm' : 'w-2.5 bg-slate-300 hover:bg-slate-400'}`} />
          ))}
        </div>
        <div className="flex gap-4 pointer-events-auto">
          <button onClick={handlePrev} disabled={currentSlide === 0}
            className="p-3.5 bg-white border border-slate-200 hover:border-primary hover:text-primary text-slate-500 disabled:opacity-30 shadow-md rounded-full transition-all">
            <ChevronLeft size={24} />
          </button>
      ...
      {/* Top Bar */}
      <div className="fixed top-0 left-0 w-full z-20 p-4 flex justify-between items-center text-sm font-mono text-slate-400 font-bold pointer-events-none">
        <div className="bg-white/50 px-4 py-1.5 rounded-full border border-slate-200 backdrop-blur-sm">ALPHA_TRAINING</div>
        <div className="flex items-center gap-2 bg-white/50 px-4 py-1.5 rounded-full border border-slate-200 backdrop-blur-sm">
